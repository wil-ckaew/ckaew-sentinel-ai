...
export class JsiSkPathEffectFactory extends Host {
  ...
  MakePath1D(path, advance, phase, style) {
    ...
    const pe = this.CanvasKit.PathEffect.MakePath1D(p, advance, phase, getEnum(this.CanvasKit, "Path1DEffect", style));
    p.delete();
    if (pe === null) {
      return null;
    }
  ...
  MakePath2D(matrix, path) {
    const p = JsiSkPath.pathFromValue(path);
    const pe = this.CanvasKit.PathEffect.MakePath2D(JsiSkMatrix.fromValue(matrix), p);
    p.delete();
    if (pe === null) {
      return null;
    }
