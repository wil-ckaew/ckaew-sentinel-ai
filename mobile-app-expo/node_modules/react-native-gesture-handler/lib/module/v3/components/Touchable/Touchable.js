...
import { isKeyboardDismissingTap, JSResponderContext } from '../ScrollViewResponderInterceptor';
...
export const Touchable = props => {
  ...
  const onFinalize = useCallback(e => {
    if (!dropKeyboardTapRef.current && pointerState.current === PointerState.INSIDE) {
      onPressOut?.(e);
    }
    if (!dropKeyboardTapRef.current && !e.canceled && !longPressDetected.current && e.pointerInside) {
      onPress?.(e);
    }
